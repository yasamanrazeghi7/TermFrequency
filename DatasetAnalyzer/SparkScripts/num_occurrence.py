...
if __name__ == "__main__":
    ...
    print(
        f"Counting {num_count} numbers occurrences in file {input_file_pattern} with window_size {window_size} and digit_limit {digit_limit}. The result will be written in {output_folder}")
    ...
    lines = lines\
        .map(json.loads)\
        .map(lambda x: x["text"])\
        .flatMap(lambda x: num_counter.num_co_occurrence(x.split(), num_count=num_count))\
        .map(lambda x: (x, 1))\
        .reduceByKey(add)
