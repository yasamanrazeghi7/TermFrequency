...
do
  ...
  $BASE/run_range_files.sh $DOMAIN $BEGIN_INDEX $((BEGIN_INDEX + CLUSTER_SHARE - 1)) > $LOG_DIR/run_log_$c.txt &
...
$BASE/run_range_files.sh $DOMAIN $BEGIN_INDEX $((TOTAL_FILES - 1)) > $LOG_DIR/run_log_$CLUSTER_COUNT.txt &
